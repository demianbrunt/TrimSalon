...
describe('BreedService', () => {
  ...
  it('should not add default breeds if they already exist', (done) => {
    const existingBreeds = [
      ...
      TestDataFactory.createBreed({ name: 'Chihuahua', size: 'small' as const }),
    ];
    
    const addSpy = spyOn(BreedService.prototype, 'add').and.returnValue(
      of({ id: 'new-id', name: 'Test', size: 'small' as const })
    );
    spyOn(BreedService.prototype, 'getData$').and.returnValue(of(existingBreeds));
    ...
    setTimeout(() => {
      ...
      expect(addedBreeds.find((b: any) => b.name === 'Labrador')).toBeUndefined();
      expect(addedBreeds.find((b: any) => b.name === 'Chihuahua')).toBeUndefined();
      done();
    }, 100);
  });
